...
function acf_php_recovery_page() {
  ...
  $acf_local = acf_get_local_field_groups();
  // process the form
  ...
  <table class="widefat">
    ...
    <tbody>
    <?php
    foreach( $acf_local as $field_group ): ?>
    <?php
     ?>
    <tr>
      <td><input type="checkbox" name="fieldsets[]" value="<?php echo esc_attr($field_group["key"]); ?>" /></td>
      <td><?php echo $field_group['title']; ?></td>
      <td><?php $matches = $wpdb->get_results("SELECT ID, post_title FROM $wpdb->posts WHERE post_title LIKE '%{$field_group['title']}%' AND post_type='".ACFPR_GROUP_POST_TYPE."'");
          if(empty($matches)) {
            echo '<em>none</em>';
          } else {
            $links = array();
